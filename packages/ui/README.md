...
## Code frame and density grid

- **`.rsk-codeframe`** — a framed code block with a header for the filename or
  language and room for a copy control. The header sits one step *darker* than
...
- **`.rsk-density`** — seven rows, column flow, five steps. Named for the shape
  ...
  inherits whichever band its subject belongs to — systems-green for database
  activity, compute-blue for build runs. That keeps it inside the taxonomy
  instead of being a green thing that means nothing.
